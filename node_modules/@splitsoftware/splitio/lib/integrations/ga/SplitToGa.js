...
var log = (0, _logger.default)('splitio-split-to-ga');
...
var SplitToGa = /*#__PURE__*/function () {
  ...
  SplitToGa.defaultMapper = function defaultMapper(_ref) {
    ...
    switch (type) {
      case _constants.SPLIT_IMPRESSION:
        return {
          hitType: 'event',
  ...
  function SplitToGa(options) {
    ...
    if (options) {
      ...
      if (Array.isArray(options.trackerNames)) this.trackerNames = (0, _lang.uniq)(options.trackerNames); // No need to validate `impressions` and `events` flags. Any other value than `false` is ignored.
  ...
  _proto.queue = function queue(data) {
    ...
    if (ga) {
      if (this.impressions === false && data.type === _constants.SPLIT_IMPRESSION) return;
      if (this.events === false && data.type === _constants.SPLIT_EVENT) return;
      var fieldsObject;
