...
var SplitCacheLocalStorage = /*#__PURE__*/function () {
  ...
  var _proto = SplitCacheLocalStorage.prototype;

  _proto.decrementCount = function decrementCount(key) {
    var count = toNumber(localStorage.getItem(key)) - 1;
    if (count > 0) localStorage.setItem(key, count);else localStorage.removeItem(key);
  };

  _proto.decrementCounts = function decrementCounts(split) {
    try {
      if (split) {
  ...
  _proto.incrementCounts = function incrementCounts(split) {
    try {
      if (split) {
        if (split.trafficTypeName) {
          var ttKey = this.keys.buildTrafficTypeKey(split.trafficTypeName);
  ...
  _proto.__checkExpiration = function __checkExpiration(expirationTimestamp) {
  ...
  _proto.__checkFilterQuery = function __checkFilterQuery() {
    var _this3 = this;
